...
| **7 Executor + auth breadth** | Async ExecutorFn + injection, `"api"` executor, Claude subscription auth via `claude setup-token` (own PKCE deliberately deferred — Phase 7 decision), model registry overlay + `/model`, `session.runner` | `eval ablate --executor api` completes seed suite sandboxed; a 401 on a subscription token surfaces the re-mint path (PROV-7 fixture) | EVP-9 (api validity, injection, container refusal), PROV-5..7, SES-5, UX-17 |
