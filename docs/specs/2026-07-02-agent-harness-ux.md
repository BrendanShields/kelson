...
- **UX-P5 — Every state comes with its verb.** Any status a user can see names the one command that acts on it (`3 proposals awaiting review → keel loop review`).
...
| Surface | Role | Notes |
|---|---|---|
| **Claude Code session** | Where work happens: `/keel:*` commands, stage flow, hook-injected context | Injected context is minimal (UX-P1); a statusline segment shows `stage · model@effort · budget` |
| **`keel` CLI/TUI** | Where the harness is operated: evals, loop review, routing, drift, signals | Ink-style component rendering per §7; every command scriptable via `--json` |
| **Repo files** | Review of record: specs, changelog, ledger, packs | PRs review them like any code |
| **OTel → external dashboards** | Metrics over time | Per ADR-0001, Keel builds no dashboards in v1 |
...
- `/keel:feature <idea>` — runs the full pipeline: ideation interview → PRD section → spec → build → verify. The spine of UC1.
- `/keel:spec`, `/keel:build`, `/keel:verify` — enter a single stage explicitly (resume, partial work).
- `/keel:status` — current task, stage, budget state, pinned lockfile.
- `/keel:accept` — explicit acceptance (TEL-7 signal).

**CLI:**

- `keel init` — install/onboard (J0).
- `keel eval ablate|compare|replay|report|suite` — the eval tool (PRD §10).
- `keel loop status|review|release|revert` — improvement-loop operations (J4).
- `keel route explain <task>` — routing transparency (PRD §11).
- `keel agents register <manifest>` — custom agent onboarding.
- `keel drift list|promote` — drift review and batched clause promotion (SPEC-8).
- `keel signals inbox|triage` — feedback-stage inbox (PIPE-1).
- `keel index rebuild` — regenerate the SQLite index from files (ERD §1).
...
1. `npx keel init` → detects Claude Code, existing config, repo type (greenfield/brownfield); shows a plan panel of what it will install (plugin, CLI, local store) and **changes nothing until confirmed**.
2. Confirmation → installs, runs a 60-second self-check (telemetry round-trip, sandbox availability), prints a "first steps" panel: greenfield → *run `/keel:feature`*; brownfield → *run excavation (J2)*.
3. First session shows the statusline segment — the only visible change to normal Claude Code use.

**Success:** first `/keel:feature` or excavation started within 30 minutes. **Edge:** no container runtime → sandbox degrades to worktree isolation with a warning badge and a link to the isolation doc (SEC-1's minimum still holds).

### J1 — Greenfield feature (P1, UC1)

1. `/keel:feature "rate-limit the public API"` → ideation interview: one question at a time (PIPE-2), each with evidence of why it's unresolved.
...
5. Verify: structured report (PIPE-8) → `/keel:accept` (or merge; acceptance then rides the correction window, TEL-7).
...
1. `keel init` in an existing repo → offers excavation with an honest cost/time estimate before starting.
...
3. Over subsequent sessions, drift alerts arrive **batched** per session end (never mid-flow, §5.4); `keel drift list` shows a survival table of inferred clauses.
4. `keel drift promote` — one screen, sorted by survival (SPEC-8), space-to-select, enter-to-promote. Promoted clauses now block per ART-4.
...
1. `keel eval ablate ponytail --suite seed` → cost/time estimate + sandbox profile shown before running (SEC-3); runs headless.
...
2. Weekly (or on `keel loop status`): summary panel — applied N (monitoring), awaiting review M, quarantined K, overhead ratio vs. cap sparkline.
3. `keel loop review` — one proposal per screen: the diff, the evidence links that motivated it (LOOP-1), its gate verdict with CIs, and monitoring status. Verbs: approve / reject / defer.
4. Auto-applied diffs appear in the changelog file (PR-reviewable); auto-reverts (LOOP-3) notify at next session start with the regression evidence and the one-command re-release path (`keel loop release <id>`).
...
1. `keel pack new` → scaffold with manifest: capability declarations (SEC-4) are required fields with inline docs, not an afterthought.
2. Local iteration: `keel eval ablate ./my-pack --suite seed` — same verdict panel as J3; the contributor sees exactly what reviewers will see.
...
1. During any build, the statusline shows the routed target per step. Curious user: `keel route explain <task>` → feature vector, chosen target, and the next candidates with estimated cost deltas (PRD §11).
...
3. Fine-tuned agent registered via `keel agents register` → immediately visible as a candidate in `route explain`, measurable via J3 from day one.
...
Never mid-flow. Batched to session end and `keel drift list`; grouped by module; inferred-clause violations visually distinct (informational) from confirmed-clause violations (blocking). The fatigue budget is explicit: if a session would show > 10 drift items, the view auto-collapses to module counts.

### 5.5 Degraded mode (KERN-1)

A single statusline badge (`degraded: telemetry`) and one line at session start. No repeated warnings. `keel doctor` names the failing component and its fix.

## 6. UX Requirements (EARS + obligations, PRD format)

- **UX-1.** Every `keel` CLI command shall support `--json` emitting schema-validated output equivalent to its rendered view.
...
- **Component set:** panel (titled box), key-value grid, table with aligned numerics, inline bar/sparkline for effect sizes and trends, side-by-side diff, select-list. Built once in `packages/cli` (Ink or equivalent — implementation plan decides), used everywhere; no ad-hoc `console.log` formatting outside the component layer.
...
Deferred, on record: when proposal volume or ledger exploration outgrows the TUI (signal: operators exporting `--json` to inspect elsewhere), `keel ui` serves a local, read-mostly web app — changelog browsing, proposal diffs with linked evidence, ledger exploration. Same files/SQLite as source of truth; the TUI's `--json` schemas (UX-1) become its API for free. No auth surface in v1-scope thinking: localhost, read-mostly, actions still via CLI.
