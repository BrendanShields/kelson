...
- **LOOP-10.** When the postmortem compiler emits a proposal cycle, it shall rank candidate proposals by expected effect and emit at most the configured edit budget — default 4 per cycle, hard schema floor 1 — clipping the remainder. Clipping is not a proposal state: a clipped candidate's evidence stays in the telemetry/findings stores and remains minable by later cycles. The edit budget is a protected threshold under LOOP-4; the loop shall have no write path to it. Bounded per-cycle edits are the textual learning rate — unbounded rewrites measurably degrade held-out performance (SkillOpt ablations, arXiv:2605.23904).
  *Obligation:* fixture cycle with 7 ranked candidates emits exactly the top 4 as `proposed` rows in rank order and no row for the clipped 3; with the budget configured to 2, exactly 2; a loop-originated diff targeting the edit-budget config is rejected at the write ACL (joins LOOP-4's matrix).
- **LOOP-11.** When the postmortem compiler assembles a proposal cycle's input, it shall include the rejection history — the 20 most recent proposals currently in `rejected` or `quarantined` state, recency by the **proposal row's `rowid`** (not the rejecting transition's time), one entry per proposal, newest first — each with id, diff target, summary, and rejection basis: the reason recorded by the transition that entered the current state (gate verdict or human override reason for `rejected`; the revert/quarantine reason for `quarantined`, never an earlier passing verdict — a reasonless entering transition records a null basis, never a predecessor transition's reason). Every emitted proposal shall record `rejections_seen_through`: the ULID id (not integer `rowid`) of the newest history entry, from **one snapshot taken at input assembly** — every proposal emitted by the cycle carries the identical value, and a rejection landing mid-cycle is invisible until the next cycle; explicitly null when the history is empty (a missing field is a schema error, distinct from null). This is the rejected-edit buffer: without it the proposer repeats failed directions (SkillOpt ablation: −1.6 to −4.6 points without the buffer; arXiv:2605.23904). Divergence-pinned (two blind readers converged on all four semantics above, 2026-07-10).
  *Obligation:* compiler-input test — with 3 rejections on record, the assembled input contains all 3 with their bases and each emitted proposal's `rejections_seen_through` equals the newest entry's ULID; with zero on record the field is null; a missing field fails schema; a 21st rejection evicts the oldest (window exactly 20, proposal-`rowid` order); a rejection inserted mid-cycle leaves every emission of that cycle carrying the snapshot watermark and appears in the next cycle's input; a quarantined proposal's entry carries its revert reason, not its pre-apply gate verdict; a proposal created earlier but rejected later than its neighbor orders by proposal `rowid`, not rejection time (the fixture must make the two orders differ); a reasonless entering transition yields a null basis even when an earlier transition recorded a reason.
...
- **LOOP-4.** The self-improvement loop shall have no write path to: kernel code/config, eval suite packs, the loop's own state-machine specification, or the safety thresholds in EVAL-2/LOOP-3 and the LOOP-10 edit budget. Changes to any of these require human approval through the normal (non-loop) contribution path.
