...
3. **Verify both replies are real readings, then compare the tables.** A reply without a committed probe table (boilerplate, tool-listing preamble, zero probes) is a dud reader — re-query it via SendMessage (its context is intact; don't respawn a fresh agent) and never compare against a dud (a reader once returned MCP preamble with 0 tool uses, 2026-07-11). Compare on the union of probe inputs (ask a follow-up via SendMessage if an agent skipped an input the other probed). **Material divergence** = any difference in return value, state, or events on the same input, excluding fields the spec declares nondeterministic (PRD §7.3 definition).
